...
25. **Cache expensive replay computations with fingerprint-based invalidation.** Functions that replay historical data (e.g., replaying all fair_line_logs through guard rails to compute per-tag WR) are correct but expensive — O(total_rows × pipeline_cost). If inputs rarely change mid-session, build an MD5 fingerprint from the input file set (file count + sizes + mtimes + config version) and cache the result to a dotfile. On cache hit, return instantly; on miss, do the full replay and write the cache. The fingerprint auto-invalidates when any input changes (new data, config bump, file edit). Add progress messages for the cold-start path so users don't think the process is hanging. Discovered in NBA v4.8.1: `load_tag_model_wr()` replayed 16,736 rows on every batch invocation (45s+ cold). Fingerprint cache reduced repeat calls from 45s → 0s.

26. **A defensive gate with zero historical impact can still be worth shipping.** When pattern analysis finds a leaky sub-population in the forced-pick bucket (e.g., SO promotes at 55% WR in a narrow profile vs 87% WR for the same population without SO), quantify the real-money impact before committing — the gate might match zero historical real-money bets. Ship it anyway if (a) the pattern is clean-edged (a single threshold cleanly separates the leaky bucket from the profitable one), (b) the pattern is likely to recur in the current regime (e.g., playoffs surface the profile more often), and (c) the gate has no downside (non-gated cases are unaffected). Document the asymmetry honestly: "0 historical real-money bets blocked; forward-looking insurance." This prevents the future mistake where the same bad pattern finally hits a real-money slate. Discovered in NBA v4.8.3: SHADOW_OVERRIDE + ECS<40 + |market|≥7 + edge<5 was 4W-6L (40%) in forced bucket vs 6W-2L (75%) at edge≥5. The gate blocked 0 real-money SO bets historically; shipped anyway because playoff regime makes the profile more likely to appear.
...
13. **Two-layer agreement is itself signal.** When the model and an independent fade evaluator agree on the same side, that agreement is a high-confidence signal — even when the Judge produces SKIP due to low edge. Forced-pick mechanisms designed to break ties (NGS flip, label flip, market fallback) destroy this signal by treating the model pick as a coin-flip default. Add a bypass: when Judge=SKIP AND Fade=AGREE in forced mode, short-circuit all SKIP-side flip rules and keep the model pick. NBA evidence (v4.8.2, NBA play-in Apr 14–17): forced 2–4 (33%) vs model 4–2 (67%) vs fade 5–1 (83%); all 4 forced losses had Fade=AGREE, with the SKIP-side selector flipping AWAY from the model+fade-validated pick. The bypass only changes behavior when the selector was actively overriding two-layer agreement — smoke test on regular-season games (n=15, 8 SKIP+AGREE matches): zero changes.
14. **Pin season type explicitly on data fetchers.** Sports stats sites often default the SeasonType filter to whichever season is currently active. When the postseason starts, fetchers that don't pin `SeasonType=Regular+Season` will silently return empty data for non-playoff teams (or playoff-only stats with tiny samples). Always pass the season filter explicitly in URL parameters; never rely on the site's default. NBA evidence (v4.8.2): nba.com defaults to Playoffs once postseason starts, breaking ratings/star-tax fetchers for the 14 non-playoff teams.
